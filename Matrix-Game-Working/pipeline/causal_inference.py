...
def get_current_action(mode="universal"):
    ...
    if mode == 'universal':
        ...
        mouse_cond = torch.tensor(CAMERA_VALUE_MAP[idx_mouse]).to(device)
        keyboard_cond = torch.tensor(KEYBOARD_IDX[idx_keyboard]).to(device)
    elif mode == 'gta_drive':
        print()
        print('-'*30)
        print("PRESS [W, S, A, D, Q] FOR MOVEMENT\n (W: forward, S: back, A: left, D: right, Q: no move)")
        ...
        device = torch.device("mps" if torch.backends.mps.is_available() else 
                            "cuda" if torch.cuda.is_available() else "cpu")
        mouse_cond = torch.tensor(CAMERA_VALUE_MAP[idx_mouse[0]]).to(device)
        keyboard_cond = torch.tensor(KEYBOARD_IDX[idx_keyboard[0]]).to(device)
    elif mode == 'templerun':
        print()
        print('-'*30)
