...
def flash_attention(
    ...
    # 移除 CUDA 硬編碼檢查 - Apple Silicon 相容性
    assert q.size(-1) <= 256
    ...
    if (version is None or version == 3) and FLASH_ATTN_3_AVAILABLE:
        ...
        if FLASH_ATTN_2_AVAILABLE:
            ...
            v_reshaped = v.view(b, lk, -1, v.size(-1))
            
            # 使用 PyTorch 的 scaled_dot_product_attention 如果可用
            if hasattr(torch.nn.functional, 'scaled_dot_product_attention'):
                x = torch.nn.functional.scaled_dot_product_attention(
                    q_reshaped.transpose(1, 2), k_reshaped.transpose(1, 2), v_reshaped.transpose(1, 2),
                    is_causal=causal, dropout_p=dropout_p, scale=softmax_scale
                ).transpose(1, 2).flatten(2)
            else:
                # 手動實現標準注意力
                scale = softmax_scale or (q.size(-1) ** -0.5)
                attention_scores = torch.matmul(q_reshaped * scale, k_reshaped.transpose(-2, -1))
                if causal:
                ...
                x = torch.matmul(attention_probs, v_reshaped).flatten(2)
