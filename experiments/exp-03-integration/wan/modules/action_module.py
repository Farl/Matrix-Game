...
try:
    ...
    def flash_attn_func(q, k, v, causal=False, dropout_p=0.0):
        """
        智能 Flash Attention 回退實現
        優先使用 PyTorch 原生，然後退到手動實現
        """
        # 方案1: 使用 PyTorch 2.0+ 的原生實現
        if hasattr(torch.nn.functional, 'scaled_dot_product_attention'):
            try:
                # 轉換形狀: flash_attn 期望 (batch, seq_len, num_heads, head_dim)
                # scaled_dot_product_attention 期望 (batch, num_heads, seq_len, head_dim)
                if len(q.shape) == 4:  # 已經是正確形狀
                    q_native = q.transpose(1, 2)  # (batch, num_heads, seq_len, head_dim)
                ...
                print(f"PyTorch 原生注意力失敗: {e}，退到手動實現")
        ...
        if dropout_p > 0.0:
            attention_probs = torch.nn.functional.dropout(attention_probs, p=dropout_p, training=True)
            
        result = torch.matmul(attention_probs, v)
        return result

# Flex attention fallback (如果不可用)
try:
    ...
    print("⚠️  Flex Attention 不可用，跳過相關功能")
